...
const DocumentVersionControl = () => {
    ...
    // Fetch documentId from localStorage on component mount
    // Fetch documentId and documentTitle from localStorage on component mount
    useEffect(() => {
        ...
        // console.log(localStorage.getItem('selectedDocumentTitle'));

        if (id) {
            // Fetch document versions based on documentId
    ...
    function handleViewVersion(versionId) {
        const token = localStorage.getItem('jwtToken');
    ...
    const handleRestoreVersion = (versionId) => {
        ...
        axios.post(`${baseURL}/api/documents/${documentId}/versions/${versionId}/restore`, {
            ...
            .then(response => {
                ...
                // Optionally refetch the versions after restoring
                axios.get(`${baseURL}/api/documents/${documentId}/versions`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                })
    ...
    const handleCompareVersions = (version1Id, version2Id) => {
        ...
        axios.get(`${baseURL}/api/documents/${documentId}/compare?version1=${version1Id}&version2=${version2Id}`, {
            ...
            .catch(error => console.error('Error comparing versions:', error));
    };
